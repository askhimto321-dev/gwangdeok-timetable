...
export const comparisonTrackKey = value => key(text(value).replace(/^(?:학생부)?(?:교과|종합)\s*[·:：]\s*/, ''));
...
export function buildComparisonRows({ compareItems = [], data = {}, caseRows = [], convertedGrade, cutoffBasis = '70', conversionGroup = '전교과', identity, evaluateMinimum }) {
  const result = [];

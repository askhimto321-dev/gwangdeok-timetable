...
export default function MinimumAchievement({db={},persist,actor,accessRole="teacher",grade="2",roster={},allRosters={},homeroomClass="",showToast}){
  ...
  return <div className="minimum-achievement minimum-dashboard-print-area" style={ui.root}>
    ...
    <header className="minimum-hero" style={ui.hero}>
      <div style={ui.heroCopy}><span style={ui.eyebrow}>담임·관리자 최종 확인</span><h2>{grade}학년 최소성취수준 현황</h2><p>반영된 과목 성적을 기준으로 학급별·과목별 주의 대상과 미도달 학생을 확인합니다.</p></div>
    ...
    <section className="minimum-rule-grid" style={ui.ruleGrid}>
      <div style={ui.ruleCard}><span style={ui.ruleIcon}><ShieldCheck size={18}/></span><div style={ui.ruleCopy}><span style={ui.commonRuleBadge}>공통과목</span><strong>출석률 2/3 이상 · 학업성취율 40% 이상</strong><small>두 기준을 모두 충족해야 도달합니다.</small></div></div>
      <div style={ui.ruleCard}><span style={{...ui.ruleIcon,...ui.electiveIcon}}><BookOpenCheck size={18}/></span><div style={ui.ruleCopy}><span style={ui.electiveRuleBadge}>선택과목</span><strong>출석률 2/3 이상</strong><small>학업성취율 40% 기준은 적용하지 않습니다.</small></div></div>
    </section>
...
const css=`
.minimum-achievement *{box-sizing:border-box}.minimum-achievement{letter-spacing:-.028em}.minimum-achievement h2,.minimum-achievement p,.minimum-achievement b,.minimum-achievement strong,.minimum-achievement small{word-break:keep-all}.minimum-achievement button,.minimum-achievement input,.minimum-achievement select{font-family:${FONT};letter-spacing:-.028em}.minimum-achievement button:disabled{cursor:not-allowed}
...
`;
const ui={
  root:{fontFamily:FONT,display:"grid",gap:14,color:"#253244"},hero:{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",alignItems:"center",justifyContent:"space-between",gap:18,padding:"20px 23px",borderRadius:20,color:"#fff",background:"linear-gradient(135deg,#2f659f,#5576ac 55%,#6f67ad)",boxShadow:"0 14px 32px rgba(49,83,145,.16)"},heroCopy:{minWidth:0,wordBreak:"keep-all"},heroActions:{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"flex-end"},eyebrow:{fontSize:11.5,fontWeight:950,opacity:.82,letterSpacing:".04em"},saveButton:{display:"inline-flex",alignItems:"center",gap:6,border:"1px solid rgba(255,255,255,.8)",borderRadius:11,padding:"10px 13px",background:"#fff",color:"#315d90",fontWeight:950,cursor:"pointer",whiteSpace:"nowrap",boxShadow:"0 7px 18px rgba(36,61,111,.14)"},printButton:{display:"inline-flex",alignItems:"center",gap:6,border:"1px solid rgba(255,255,255,.42)",borderRadius:11,padding:"10px 13px",background:"rgba(255,255,255,.13)",color:"#fff",fontWeight:950,cursor:"pointer",whiteSpace:"nowrap"},saveButtonDisabled:{opacity:.45},ruleGrid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10},ruleCard:{display:"flex",gap:12,alignItems:"center",padding:"13px 15px",border:"1px solid #d9e3ef",borderRadius:15,background:"linear-gradient(135deg,#fff,#f8fbff)",boxShadow:"0 5px 16px rgba(55,72,110,.04)"},ruleCopy:{display:"grid",gridTemplateColumns:"auto minmax(0,1fr)",gap:"3px 9px",alignItems:"center",minWidth:0},commonRuleBadge:{gridRow:"1 / span 2",display:"inline-flex",alignItems:"center",borderRadius:999,padding:"5px 8px",color:"#315d90",background:"#e8f1fb",fontSize:10.5,fontWeight:950,whiteSpace:"nowrap"},electiveRuleBadge:{gridRow:"1 / span 2",display:"inline-flex",alignItems:"center",borderRadius:999,padding:"5px 8px",color:"#6a4f87",background:"#f1eafa",fontSize:10.5,fontWeight:950,whiteSpace:"nowrap"},ruleIcon:{width:38,height:38,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:12,color:"#3568a3",background:"#e8f1fb"},electiveIcon:{color:"#6a4f87",background:"#f1eafa"},section:{border:"1px solid #d9e3ee",borderRadius:17,padding:16,background:"#fff",boxShadow:"0 7px 20px rgba(55,72,110,.05)"},filterHeader:{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",marginBottom:12},roleBadge:{border:"1px solid #c8d9ea",borderRadius:999,padding:"6px 9px",color:"#315d90",background:"#eef5fc",fontSize:10.5,fontWeight:900},toolbar:{display:"flex",gap:8,alignItems:"stretch",flexWrap:"wrap",marginBottom:12},search:{display:"flex",alignItems:"center",gap:9,flex:"1 1 280px",minHeight:46,border:"1.5px solid #c8d7e8",borderRadius:14,padding:"6px 12px",background:"linear-gradient(180deg,#fff,#f8fbff)"},searchIcon:{width:30,height:30,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:9,color:"#3568a3",background:"#e9f2fc"},metrics:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(125px,1fr))",gap:8},metric:{display:"grid",gap:3,padding:"11px 12px",border:"1px solid #e0e6ef",borderRadius:12,background:"#fafbfd"},metric_good:{borderColor:"#c5e2cf",background:"#f0f8f3"},metric_warn:{borderColor:"#ecd59d",background:"#fff9ea"},metric_bad:{borderColor:"#ecc6c1",background:"#fff4f2"},bulkSection:{background:"linear-gradient(135deg,#fbfdff,#f7fbff)"},bulkHeader:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:13},bulkIcon:{width:36,height:36,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:11,color:"#315d90",background:"#e8f1fb"},bulkGrid:{display:"grid",gridTemplateColumns:"150px minmax(240px,1fr) minmax(320px,1.4fr) 200px",gap:12,alignItems:"start"},bulkColumn:{display:"grid",gap:8,minWidth:0},bulkSubjectColumn:{},bulkStudentColumn:{},checkGrid:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(145px,1fr))",gap:6,maxHeight:180,overflowY:"auto"},bulkStudentHead:{display:"flex",justifyContent:"space-between",gap:8,alignItems:"center"},miniSearch:{display:"flex",alignItems:"center",gap:6,border:"1px solid #d1dce9",borderRadius:10,padding:"0 8px",background:"#fff"},studentCheckGrid:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(145px,1fr))",gap:6,maxHeight:180,overflowY:"auto"},bulkApplyButton:{border:0,borderRadius:10,padding:"10px 11px",color:"#fff",background:"#3568a3",fontWeight:950,cursor:"pointer"},attentionPanel:{border:"1px solid #edc8c2",borderRadius:16,padding:15,background:"linear-gradient(135deg,#fff8f6,#fffdf9)"},attentionHead:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,color:"#8f453a"},attentionList:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:7,marginTop:10},attentionChip:{display:"grid",gap:2,padding:"9px 10px",border:"1px solid #ecd4cf",borderRadius:11,background:"#fff"},attentionChipFail:{borderColor:"#e8b9b2",background:"#fff4f2"},moreButton:{display:"inline-flex",alignItems:"center",gap:5,marginTop:10,border:"1px solid #d8c0bb",borderRadius:9,padding:"7px 10px",color:"#875045",background:"#fff",fontWeight:900,cursor:"pointer"},tableTitle:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:10,color:"#344b67"},tableWrap:{overflowX:"auto",border:"1px solid #cfd9e5",borderRadius:14,background:"#fff",boxShadow:"0 5px 16px rgba(47,70,103,.04)"},attendanceEdit:{display:"grid",gridTemplateColumns:"112px minmax(125px,1fr)",gap:5},missing:{display:"grid",gap:3,color:"#9c493b"},noRows:{padding:24,textAlign:"center",color:"#778497",fontWeight:800},empty:{minHeight:240,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,border:"1px dashed #d5deea",borderRadius:15,color:"#738095",background:"#fafbfd"}
};

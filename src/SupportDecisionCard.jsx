...
export default function SupportDecisionCard({item,index,studentGrade,cutoffBasis,student,busy,onRemove,onOpenCases}) {
  ...
  return <article className="susi-beta-plan-card kd-decision-card">
    <header><div><span className="kd-decision-type">{item.stored.admissionType || '전형 확인'} · {item.stored.track}</span><h4>{item.stored.university}</h4><p>{item.stored.department}</p></div><span className="kd-decision-number">{index+1}</span></header>
    <div className="kd-decision-primary">
      <section className="kd-decision-grade"><h5>내신 컷 비교 <span>2026 공개 결과</span></h5><div className="kd-decision-numbers"><div><small>학생 9등급 환산</small><b>{fmt(studentGrade)}</b></div><span aria-hidden="true">↔</span><div><small>{cutoffBasis}%컷</small><b>{fmt(cut)}</b></div></div><p><strong>{item.support?.label || '판정 자료 없음'}</strong>{difference!=null && <span>학생 − 컷 {difference>0?'+':''}{difference.toFixed(2)}</span>}</p><small>50% {fmt(item.admissionItem?.[1])} · 70% {fmt(item.admissionItem?.[2])} · 등급은 낮을수록 유리</small></section>

...
function ContactWorkspace({ data, accessRole, homeroomClass, showToast, onUpdateData, actor }) {
  ...
  const applyRoleToSelected = () => {
    if (!selected.length) return showToast("학생회 역할을 적용할 학생을 먼저 선택해주세요.", "error");
  ...
  const saveRoleDraft = () => {
    try {
      ...
      showToast(`학생회 역할 변경 ${dirtyRoleCount}건을 임시 저장했습니다.`, "success");
    } catch {
      showToast("학생회 역할 임시 저장에 실패했습니다.", "error");
    }
  };
  const applyRoleDraft = async () => {
    if (!dirtyRoleCount) return showToast("학교에 반영할 학생회 역할 변경사항이 없습니다.", "error");
    const nextContacts = contacts.map(item => normalizeContactItem({ ...item, leadershipRole: roleValue(item) }));
    const ok = await onUpdateData({ ...data, contacts: nextContacts }, `학생회 역할 ${dirtyRoleCount}건을 학교 자료에 반영했습니다.`);
    if (ok) {
  ...
  return <div className="gdt-contact-workspace">
    ...
    <div className="gdt-role-manager">
      <div><b>학생회 역할 일괄 설정</b><small>행의 역할을 직접 바꾸거나, 선택 학생에게 같은 역할을 한 번에 적용한 뒤 저장·반영합니다.</small></div>
      <select value={bulkLeadershipRole} onChange={event=>setBulkLeadershipRole(event.target.value)}>{LEADERSHIP_ROLE_OPTIONS.map(value=><option key={value}>{value}</option>)}</select>
      ...
      {filtered.map(item => {
        ...
        return <tr key={item.sid} className={editing ? "editing" : ""}>
          {editing ? <>
            ...
            <td><select className="gdt-role-select" value={roleValue(item)} onChange={event=>setRoleValue(item.sid,event.target.value)}>{LEADERSHIP_ROLE_OPTIONS.map(value=><option key={value}>{value}</option>)}</select></td>

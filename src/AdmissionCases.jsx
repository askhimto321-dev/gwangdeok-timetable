...
.admission-clear-button{display:inline-flex!important;align-items:center!important;justify-content:center!important;gap:5px!important;min-height:32px!important;padding:6px 10px!important;border:1px solid #e39a9a!important;border-radius:8px!important;background:#fff0f0!important;color:#c83232!important;font-size:12px!important;font-weight:950!important;cursor:pointer!important;box-shadow:0 3px 8px rgba(190,48,48,.07)!important}
.admission-clear-button:hover{background:#ffe5e5!important;border-color:#d97878!important;color:#b82222!important}
...
/* Ver13: compact 9-grade-only student benchmark */
.admission-case-search-actions{
  grid-template-columns:minmax(150px,180px) minmax(300px,1fr) minmax(118px,132px) minmax(164px,178px)!important;
}
.admission-case-student-benchmark{
  display:grid!important;
...
.admission-case-benchmark-head{grid-column:auto!important}
.admission-case-benchmark-head>small{font-size:9.2px;color:#6c7f96;font-weight:900;white-space:nowrap}
.admission-case-benchmark-primary{display:flex!important;align-items:end;justify-content:space-between;gap:8px}
.admission-case-benchmark-primary strong{font-size:22px!important}
.admission-case-benchmark-primary small{padding-bottom:2px}
.admission-case-benchmark-companion,.admission-grade-system-toggle,.admission-case-student-benchmark em{display:none!important}
@media(max-width:1220px){
  .admission-case-search-actions{grid-template-columns:minmax(150px,180px) minmax(290px,1fr) minmax(118px,132px) minmax(164px,178px)!important}
}
@media(max-width:760px){
...
/* Ver16: 대입 비교·필터·전형 통계 정돈 */
.admission-comparison-summary{align-items:stretch!important}
.admission-comparison-summary>div{display:grid!important;align-content:center!important;gap:5px!important;min-height:74px!important;padding:11px 13px!important}
.admission-comparison-summary small{font-size:10.5px!important;font-weight:850!important;color:#6d7b8d!important;line-height:1.25!important}
.admission-comparison-summary b{font-size:19px!important;line-height:1.05!important;font-weight:950!important;font-variant-numeric:tabular-nums}
.admission-quick-filter{align-items:stretch!important}
.admission-quick-filter-group{align-items:center!important;min-height:70px}
.admission-quick-filter-label{align-content:center!important;gap:4px!important}
.admission-quick-filter-label b{font-size:14.5px!important}.admission-quick-filter-label span{font-size:10.5px!important;line-height:1.35!important;color:#63758d!important}
.admission-quick-filter-group:nth-child(2) .admission-quick-filter-label span{color:#9a5f22!important;font-weight:900!important}
.admission-quick-filter-group:first-child .admission-quick-filter-options{display:grid!important;grid-template-columns:repeat(4,minmax(62px,1fr))!important;width:100%}
.admission-quick-filter-group:nth-child(2) .admission-quick-filter-options{display:grid!important;grid-template-columns:repeat(6,minmax(62px,1fr))!important;width:100%}
.admission-quick-filter-options>button{width:100%;min-width:0!important}
@media(max-width:980px){.admission-quick-filter-group:first-child .admission-quick-filter-options{grid-template-columns:repeat(4,minmax(58px,1fr))!important}.admission-quick-filter-group:nth-child(2) .admission-quick-filter-options{grid-template-columns:repeat(3,minmax(72px,1fr))!important}}
@media(max-width:560px){.admission-quick-filter-group:first-child .admission-quick-filter-options{grid-template-columns:repeat(2,minmax(72px,1fr))!important}.admission-quick-filter-group:nth-child(2) .admission-quick-filter-options{grid-template-columns:repeat(2,minmax(72px,1fr))!important}}
.admission-support-filter{min-height:44px!important;padding:6px 10px!important}.admission-support-filter b{font-size:11.5px!important}.admission-support-filter small{font-size:9px!important;line-height:1.2!important}
.admission-case-student-benchmark{min-width:205px!important;gap:7px!important;padding:10px 12px!important}
.admission-case-benchmark-head{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:9px!important}
.admission-grade-system-toggle{display:inline-flex!important;gap:3px!important;padding:3px!important;border-radius:999px;background:#edf2f8;border:1px solid #d7e1ec}
.admission-grade-system-toggle button{border:0;background:transparent;color:#65758a;border-radius:999px;padding:4px 7px;font-size:9px;font-weight:950;cursor:pointer;white-space:nowrap}
.admission-grade-system-toggle button.is-active{background:#315f95;color:#fff;box-shadow:0 2px 6px rgba(49,95,149,.22)}.admission-grade-system-toggle button:disabled{opacity:.35}
.admission-case-student-benchmark em{display:block!important;font-style:normal;font-size:8.8px;color:#7a899b;font-weight:750}
.admission-vertical-shell{display:grid;gap:12px;height:100%}
.admission-vertical-rate{display:grid!important;gap:1px!important}.admission-vertical-rate b{font-size:11px;color:#315f95}.admission-vertical-rate span{font-size:8.7px;color:#7c899a}
.admission-vertical-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;padding:10px;border-radius:12px;background:#f5f8fc;border:1px solid #dfe6ef}
.admission-vertical-summary>div{display:grid;gap:2px;padding:8px 9px;border-radius:9px;background:#fff;border:1px solid #e4e9ef;min-width:0}
.admission-vertical-summary small{font-size:9.5px;color:#718197;font-weight:850}.admission-vertical-summary b{font-size:13px;color:#294969;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
@media(max-width:680px){.admission-vertical-summary{grid-template-columns:1fr}.admission-case-student-benchmark{min-width:0!important;width:100%}}

.admission-link-button.is-navi{background:#edf4fb!important;border-color:#bfd0e3!important;color:#315f91!important}
.admission-link-button.is-plan{background:#315f91!important;border-color:#315f91!important;color:#fff!important}
.admission-row-actions{display:flex;align-items:center;justify-content:center;gap:4px;flex-wrap:wrap}
.admission-row-actions button,.admission-mini-link{border:1px solid #cfd9e5;border-radius:7px;background:#fff;color:#315f91;padding:4px 6px;font-size:9px;font-weight:900;cursor:pointer;white-space:nowrap}
.admission-row-actions button.is-plan,.admission-mini-link.is-plan{background:#eef3fb;border-color:#bdcfe3;color:#264f7a}
.admission-case-row-tools{display:flex!important;align-items:center;justify-content:center;gap:3px;flex-wrap:wrap}
@media print{
...
.admission-type-performance-shell{display:grid;grid-template-columns:minmax(0,1fr) 178px;gap:12px;align-items:stretch;min-height:0}
.admission-type-performance-list{display:grid;gap:7px;align-content:start;padding:11px;border:1px solid #dce5ef;border-radius:13px;background:linear-gradient(180deg,#fbfdff,#f7f9fc)}
.admission-type-performance-row{display:grid;grid-template-columns:minmax(88px,116px) minmax(90px,1fr) 92px 58px;gap:8px;align-items:center;min-width:0;padding:5px 0;border-bottom:1px solid #edf1f5}
.admission-type-performance-row:last-child{border-bottom:0}
.admission-type-performance-label{font-size:11.4px;font-weight:950;color:#263a51;line-height:1.28;word-break:keep-all;overflow-wrap:anywhere}
.admission-type-performance-bars{display:grid;gap:5px;min-width:0}
.admission-type-performance-track{height:8px;border-radius:999px;background:#e9e6df;overflow:hidden}
.admission-type-performance-track span{display:block;height:100%;border-radius:inherit}
.admission-type-performance-track.support span{background:linear-gradient(90deg,#7198cc,#4b78b3)}
.admission-type-performance-track.accept span{background:linear-gradient(90deg,#7db28f,#4c8761)}
.admission-type-performance-counts{display:grid;grid-template-columns:1fr;gap:2px;white-space:nowrap}
.admission-type-performance-counts span{font-size:9.5px;color:#718096;font-weight:850}.admission-type-performance-counts b{font-size:11px;color:#294f7f;font-weight:950}
.admission-type-performance-rate{display:grid;justify-items:end;gap:0;min-width:55px;color:#2f6d4b}.admission-type-performance-rate b{font-size:12.5px;line-height:1.1;font-weight:950}.admission-type-performance-rate small{font-size:8.7px;font-weight:850;color:#718096}
.admission-type-summary{display:grid;grid-template-columns:1fr;gap:7px;align-content:start;padding:10px;border:1px solid #dce4ee;border-radius:13px;background:#f4f7fb}
.admission-type-summary>div{display:grid;gap:3px;padding:10px 11px;border-radius:10px;background:#fff;border:1px solid #e0e7ef;min-width:0}
.admission-type-summary small{font-size:9.7px;color:#718197;font-weight:850}.admission-type-summary b{font-size:14px;color:#294969;line-height:1.2;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.admission-type-summary em{font-size:11px;color:#2f6d4b;font-style:normal;font-weight:950}
.admission-overview-pair{grid-template-columns:minmax(590px,1.25fr) minmax(370px,.75fr)!important}
...
.admission-case-conversion-group select{font-size:12.5px!important;font-weight:900!important}.admission-case-conversion-note{font-size:10.8px!important}
.admission-case-link-fallback{display:flex;align-items:flex-start;gap:8px;margin-top:8px;padding:10px 12px;border:1px solid #ecd2a9;border-radius:10px;background:#fff8ea;color:#77551d;font-size:11px;line-height:1.5}.admission-case-link-fallback svg{flex:0 0 auto;margin-top:1px}.admission-case-link-fallback span{display:grid;gap:2px}.admission-case-link-fallback b{font-size:11.5px;font-weight:950}

/* Patch 43: 지역 통계 균형 및 표본 규모 단계별 강조 */
.admission-overview-pair{grid-template-columns:repeat(2,minmax(0,1fr))!important;align-items:start!important}
.admission-region-bars{grid-template-columns:repeat(2,minmax(0,1fr))!important;column-gap:14px!important;row-gap:8px!important;align-content:start}
.admission-region-bar-row{grid-template-columns:58px minmax(74px,1fr) auto!important;gap:7px!important;padding:3px 0;min-width:0}
...
@media(max-width:860px){.admission-overview-pair,.admission-region-bars{grid-template-columns:1fr!important}}
...
function caseSufficiency(total){
  if(total>=20)return{label:"표본 많음",detail:"20건 이상",level:"many"};
  if(total>=10)return{label:"표본 적당",detail:"10~19건",level:"adequate"};
  return{label:"표본 적음",detail:total>=5?"5~9건":"4건 이하 · 해석 주의",level:"low"};
}
...
function gradeDifference(studentGrade,cutGrade){
  ...
  if(Math.abs(diff)<0.05)return{label:"±0.0",detail:"합격자 전교과 50%컷과 유사",color:"#315a9b",background:"#edf3ff"};
...
function caseUniversityMatches(row,target,targetRegion=""){
  const targetBase=caseUniversityBase(target);
  if(!targetBase)return true;
  const rowBase=caseUniversityBase(row?.universityNormalized||row?.university);
  if(rowBase!==targetBase)return false;
  const targetCampus=caseCampusLabel(target,targetRegion);
  if(!targetCampus)return true;
  return caseCampusForRow(row)===targetCampus;
}
...
function applicationBand(studentGrade,cutGrade){
  const result = supportBandValue(studentGrade, cutGrade);
  if (!result) return {label:"판단 보류",detail:"50%컷 자료 부족",color:"#656b75",background:"#f1f3f5",border:"#d9dde2"};
  const meta = {
    상향: {detail:"50%컷보다 0.5등급 초과 뒤",color:"#9c3f45",background:"#fff0f1",border:"#efc8cb"},
    소신: {detail:"50%컷보다 0.2등급 초과~0.5등급 뒤",color:"#9a5f22",background:"#fff4e7",border:"#edcfaa"},
    적정: {detail:"50%컷 ±0.2등급",color:"#6f5a17",background:"#fff8d9",border:"#eadb91"},
    안정: {detail:"50%컷보다 0.2등급 초과~0.5등급 앞",color:"#2f6d4b",background:"#edf8f1",border:"#c7e3d1"},
    하향: {detail:"50%컷보다 0.5등급 초과 앞",color:"#365a8a",background:"#edf3ff",border:"#cad8ef"},
  };
  return {...result, ...meta[result.label]};
}
...
function supportPlanFromCase(row={}, universityOverride=""){
  const rawType=String(row?.detailType||row?.type||row?.admissionType||"").trim();
  const broad=broadAdmissionGroup({...row,detailType:rawType});
  return{university:universityOverride||row?.universityNormalized||row?.university||"",region:row?.region||"",department:row?.department||"",field:row?.field||"",admissionType:broad,track:rawType||broad,detailType:rawType,source:"광덕고 별도 사례",sourceCaseId:row?.caseId||""};
}
...
function emptyCaseFilters(){return{query:"",regions:[],fields:[],schoolTypes:[],admissionGroups:[],detailTypes:[],results:[],bands:[],minGrade:"",maxGrade:""}}
...
function AdmissionMethodFilter({cases,groups=[],detailTypes=[],onGroupsChange,onDetailsChange}){
  ...
  return <div className="admission-method-filter">
    ...
    {(groups.length||detailTypes.length)?<div className="admission-detail-selected">{groups.map(item=><button type="button" key={`group-${item}`} onClick={()=>toggleGroup(item)}>{item} ×</button>)}{detailTypes.map(item=><button type="button" key={item} onClick={()=>onDetailsChange(detailTypes.filter(value=>value!==item))}>{item} ×</button>)}<button type="button" className="admission-clear-button" onClick={()=>{onGroupsChange([]);onDetailsChange([])}}>전형 필터 전체 해제</button></div>:null}
  </div>;
}
...
function StudentMatch({rows,profile,favorites=[],onToggleFavorite,onOpenAdmission,onOpenSusiNavi,onAddSupportPlan,onOpenSupportPlan,admissionRows=[],selectedUniversity="",onSelectUniversity,onBackUniversity,onOpenDepartmentCases,quickFilters=null,supportBands=[]}){
  ...
  const visibleSimilar=useMemo(()=>{
    ...
    return similar.filter(row=>{
      const caseGrade=row.overallGrade;
      ...
      <section style={styles.universityDetailPage}>
        <div style={styles.detailHero}>
          ...
          <div className="admission-detail-compare" style={styles.detailStudentCompare}>
            ...
            <div><small>합격자 전교과 50%컷</small><b>{fmt(cut50)}</b></div>
...
function Bars({rows}){
  ...
  return <div className="admission-region-bars" style={{display:"grid",gap:12}}>{rows.map(row=><div className="admission-region-bar-row" key={row.label} style={styles.barRowNew}>
...
function VerticalBars({rows}){
  ...
  const supportTotal=rows.reduce((sum,row)=>sum+Number(row.total||0),0);
  ...
  return <div className="admission-type-performance-shell">
    <div className="admission-type-performance-list">{rows.map(row=>{const rate=row.total?row.accepted/row.total*100:0;return <div className="admission-type-performance-row" key={row.label}>
      ...
      <div className="admission-type-performance-counts"><span>지원 <b>{row.total}</b></span><span>합격 <b>{row.accepted}</b></span></div>
      <div className="admission-type-performance-rate"><b>{fmt(rate,1)}%</b><small>합격률</small></div>
    </div>})}</div>
    <aside className="admission-type-summary">
      ...
      <div><small>최고 합격률</small><b>{best?best.label:"-"}</b><em>{best?`${fmt(best.accepted/best.total*100,1)}%`:""}</em></div>
    </aside>
  </div>;
}
...
function CaseSearch({rows,comparisonRows=[],profile,favorites=[],onToggleFavorite,onOpenSusiNavi,onAddSupportPlan,onOpenSupportPlan,focusUniversity="",focusDepartment="",focusAdmissionType="",onClearLinkedFocus,globalMinGrade="",globalMaxGrade=""}){
  ...
  const focusResolution=useMemo(()=>{
    if(!focusUniversity)return{rows,scope:"none",message:""};
    const source=(comparisonRows?.length?comparisonRows:rows)||[];
    const universityRows=source.filter(row=>caseUniversityMatches(row,focusUniversity));
    if(!universityRows.length)return{rows:[],scope:"none",message:`${focusUniversity}와 연결되는 광덕고 사례를 찾지 못했습니다.`};
    const departmentRows=focusDepartment?universityRows.filter(row=>caseDepartmentMatches(row.department,focusDepartment)):universityRows;
    const typeRows=effectiveFocusAdmissionType?departmentRows.filter(row=>normalizeSearchText(`${row.admissionType} ${row.detailType}`).includes(normalizeSearchText(effectiveFocusAdmissionType))):departmentRows;
    if(typeRows.length)return{rows:typeRows,scope:"exact",message:""};
    if(departmentRows.length)return{rows:departmentRows,scope:"department",message:`연결한 전형명이 광덕고 자료와 달라 ${focusDepartment||"선택 모집단위"}의 전체 전형 ${departmentRows.length.toLocaleString()}건을 표시합니다.`};
    return{rows:universityRows,scope:"university",message:`연결한 모집단위명이 광덕고 자료와 달라 ${focusUniversity} 대학 전체 ${universityRows.length.toLocaleString()}건을 표시합니다.`};
  ...
  return <div className="admission-case-search-root admission-case-print-root">
    <div className="admission-case-search-controls no-print">
      <SearchBox value={query} onChange={setQuery} placeholder="예: 건국대 경영 · 대학/학과/전형 통합 검색" count={`${sorted.length.toLocaleString()}건`}/>
...
export function AdmissionCaseAnalytics({gdb,roster={},currentGrade="2",selectedStudentSid,onSelectedStudentSidChange,selectedStudentQuery,onSelectedStudentQueryChange,favorites=[],onToggleFavorite,onOpenAdmission,onOpenSusiNavi,onAddSupportPlan,onOpenSupportPlan,focusUniversity="",focusDepartment="",focusAdmissionType="",onBackToConsultation,onClearFocus}){
  ...
  const applyView=view=>{setTab(view?.tab||"student");setStudentUniversity(view?.studentUniversity||"");setUniversitySelection(view?.universitySelection||"");setSearchFocus(view?.searchFocus||{university:"",department:"",admissionType:""});setFilters(view?.filters||emptyCaseFilters())};
  useEffect(()=>{currentView.current=viewSnapshot()},[tab,studentUniversity,universitySelection,searchFocus,filters]);
  useEffect(()=>{
    if(typeof window==="undefined")return undefined;
    const session=caseHistorySessionRef.current;
    const initial=viewSnapshot();
    window.history.replaceState({...window.history.state,kdAdmissionCaseSession:session,kdAdmissionCaseDepth:0,kdAdmissionCaseView:initial},"");
    setNavDepth(0);
    const handler=event=>{const state=event.state||{};if(state.kdAdmissionCaseSession!==session||!state.kdAdmissionCaseView)return;setNavDepth(Math.max(0,Number(state.kdAdmissionCaseDepth||0)));applyView(state.kdAdmissionCaseView)};
    window.addEventListener("popstate",handler);
    return()=>window.removeEventListener("popstate",handler);
  },[]);
  useEffect(()=>{setStudentUniversity("");setFilters(emptyCaseFilters())},[sid]);
  const navigate=view=>{
    const next={...currentView.current,...view};
    if(JSON.stringify(next)===JSON.stringify(currentView.current))return;
    const nextDepth=navDepth+1;
    if(typeof window!=="undefined")window.history.pushState({...window.history.state,kdAdmissionCaseSession:caseHistorySessionRef.current,kdAdmissionCaseDepth:nextDepth,kdAdmissionCaseView:next},"");
    setNavDepth(nextDepth);applyView(next);
  };
  const goBack=()=>{if(typeof window!=="undefined"&&navDepth>0)window.history.back();else applyView({...currentView.current,tab:"student",studentUniversity:"",universitySelection:"",searchFocus:{university:"",department:"",admissionType:""},filters:emptyCaseFilters()})};
  useEffect(()=>{
    if(!focusUniversity)return;
    const base=String(focusUniversity).trim();
    const next=focusDepartment||focusAdmissionType
...
smallButton:{display:"inline-flex",alignItems:"center",gap:6,border:"1px solid #e39a9a",borderRadius:9,background:"#fff0f0",padding:"9px 12px",fontSize:12.5,fontWeight:950,cursor:"pointer",color:"#c83232",boxShadow:"0 3px 8px rgba(190,48,48,.07)"},

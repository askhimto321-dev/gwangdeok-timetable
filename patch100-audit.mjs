...
try {
 ...
 fs.writeFileSync('../Patch100_minimum_audit.csv','\ufeff'+csv.map(row=>row.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(',')).join('\r\n'));
 ...
 if(process.env.AUDIT_BROWSER!=='1'){console.log('SSR assertions passed; browser preview not requested');} else {
 ...
 await page.screenshot({path:'../audit/patch96-diagnosis.png',fullPage:true});

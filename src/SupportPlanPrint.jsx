...
export function SupportPlanReport({items,student,studentGrade,cutoffBasis}) {
  ...
  return <main className={`kd-print-doc is-count-${printedCount}`}>
    <header className="kd-print-head">
      ...
      <p>9등급 환산 {grade(studentGrade)} · {cutoffBasis}%컷 · {new Date().toLocaleDateString('ko-KR')}</p>
    </header>
    ...
    <div className="susi-beta-workspace">
      <div className="susi-beta-plan-grid">{printableItems.map(({item,index})=><SupportDecisionCard key={index} item={item} index={index} studentGrade={studentGrade} cutoffBasis={cutoffBasis} student={student} printMode/>)}</div>
    </div>

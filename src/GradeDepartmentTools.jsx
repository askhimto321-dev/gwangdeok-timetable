...
function ContactWorkspace({ data, accessRole, homeroomClass, showToast, onUpdateData, actor }) {
  ...
  const printContacts = () => {
    const ok = openPrintDocument({
      ...
      headers: ["반", "번호", "학번", "성명", "학적", "학급/학생 자치", "학생 연락처", "보호자 연락처", "비고"],
  ...
  const renderEditorCells = () => <>
    ...
    <td><select value={editRow.leadershipRole || "없음"} onChange={event=>setEditRow({...editRow,leadershipRole:event.target.value})}>{leadershipOptionsFor(editRow.leadershipRole).map(value=><option key={value}>{value}</option>)}</select></td>
      ...
      {filtered.map(item => {
        ...
        return <tr key={item.sid} className={editing ? "editing" : ""}>
          {editing ? <>
            ...
            <td><select className="gdt-role-select" value={roleValue(item)} onChange={event=>setRoleValue(item.sid,event.target.value)}>{leadershipOptionsFor(roleValue(item)).map(value=><option key={value}>{value}</option>)}</select></td>

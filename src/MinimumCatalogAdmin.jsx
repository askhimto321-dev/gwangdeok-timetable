...
export default function MinimumCatalogAdmin({gdb,persistGrades,showToast}) {
 ...
 return <section className="kd-minimum-admin">
  ...
  <div className="kd-minimum-actions"><label>해결 유형 <select aria-label="최저자료 해결유형" value={dispositionFilter} onChange={e=>{setDispositionFilter(e.target.value);setCatalogPage(0);}}><option value="전체">전체 유형</option>{dispositions.map(i=><option key={i.code} value={i.code}>{i.label} ({i.count})</option>)}</select></label><label>검토 사유 <select aria-label="최저자료 검토사유" value={issueFilter} onChange={e=>{setIssueFilter(e.target.value);setCatalogPage(0);}}><option value="전체">전체 사유</option>{issueCounts.map(i=><option key={i.code} value={i.code}>{i.label} ({i.count})</option>)}</select></label><small>해결 유형은 우선 조치 기준이며, 검토 사유는 중복될 수 있습니다.</small></div>
  <div className="kd-minimum-actions"><label>세부 작업 <select aria-label="최저자료 세부작업" value={workTypeFilter} onChange={e=>{setWorkTypeFilter(e.target.value);setCatalogPage(0);}}><option value="전체">전체 작업</option>{workTypes.map(i=><option key={i.code} value={i.code}>{i.label} ({i.count})</option>)}</select></label><small>현재 표시 자료의 매핑·규칙 작업을 유형별로 분류합니다.</small></div>
  <div className="kd-minimum-actions"><input aria-label="최저자료 검색" placeholder="대학·전형·모집단위·검토사유 검색" value={query} onChange={e=>{setQuery(e.target.value);setCatalogPage(0);}}/><select aria-label="최저자료 검토상태" value={filter} onChange={e=>{setFilter(e.target.value);setCatalogPage(0);}}>{['전체','계산가능','검토필요','사용안함'].map(s=><option key={s}>{s}</option>)}</select><span>{filtered.length}건 · {filtered.length?`${catalogPage*100+1}–${Math.min(filtered.length,(catalogPage+1)*100)}`:'0'}건 표시</span>{catalogPage>0&&<button type="button" onClick={()=>setCatalogPage(p=>p-1)}>이전 100건</button>}{(catalogPage+1)*100<filtered.length&&<button type="button" onClick={()=>setCatalogPage(p=>p+1)}>다음 100건</button>}</div>
  <div className="kd-minimum-table"><table><thead><tr><th>학년도·대학</th><th>전형·범위</th><th>최저 조건</th><th>상태·확인 사항</th><th>출처</th></tr></thead><tbody>{filtered.slice(catalogPage*100,(catalogPage+1)*100).map(r=>{const disposition=minimumReviewDisposition(r),work=minimumMappingPlan(r)||minimumRulePattern(r);return <tr key={r.id}><td>{r.admissionYear}<br/><b>{r.university}{r.campus?`(${r.campus})`:''}</b></td><td>{r.admissionType} · {r.track}<br/>{r.department}{r.excluded&&<small>제외: {r.excluded}</small>}</td><td>{r.ruleText}{r.note&&<small>{r.note}</small>}</td><td><b className={r.reviewStatus==='계산가능'?'is-ready':'is-review'}>{r.reviewStatus}</b>{r.reviewStatus==='검토필요'&&<small><strong>{disposition.label}</strong>: {disposition.action}</small>}{work&&<small><strong>{work.label}</strong>: {work.action}<br/>필요 정보: {work.required}</small>}<small>{r.reviewReason||'원문에서 구조화한 조건'}</small>{minimumReviewIssues(r).map(i=><small key={i.code}><strong>{i.label}</strong>: {i.action}</small>)}<small>{r.id}</small></td><td>{r.source}<br/>{r.page}쪽{/^https?:\/\//.test(r.resolutionSource||'')&&<small><a href={r.resolutionSource} target="_blank" rel="noopener noreferrer">확인한 공식 자료 ↗</a></small>}</td></tr>})}</tbody></table>{!filtered.length&&<p>검색 결과가 없습니다.</p>}</div>

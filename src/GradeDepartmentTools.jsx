...
function studentPlacementFromSid(value) {
  const sid = normalizeSid(value);
  if (!/^\d{5}$/.test(sid)) return { classNumber: null, number: null };
  const classNumber = Number(sid.slice(1, 3));
  const number = Number(sid.slice(3, 5));
  return {
    classNumber: classNumber >= 1 && classNumber <= 20 ? classNumber : null,
    number: number >= 1 && number <= 60 ? number : null,
  };
}

function normalizeContactItem(item) {
  const sid = normalizeSid(item?.sid);
  const sidPlacement = studentPlacementFromSid(sid);
  const rawClass = Number(item?.classNumber);
  const rawNumber = Number(item?.number);
  return {
    ...item,
    sid,
    name: cleanText(item?.name),
    classNumber: sidPlacement.classNumber || (rawClass >= 1 && rawClass <= 20 ? rawClass : null),
    number: sidPlacement.number || (rawNumber >= 1 && rawNumber <= 60 ? rawNumber : null),
...
function parseContacts(workbook) {
  ...
  const classIndex = header.findIndex(cell => cleanText(cell) === "반");
  const numberIndex = header.findIndex(cell => cleanText(cell) === "번호");
  ...
  return rows.slice(headerIndex + 1).map(row => {
    ...
    return normalizeContactItem({
      sid,
      name,
      classNumber: sidPlacement.classNumber || Number(row[classIndex]) || null,
      number: sidPlacement.number || Number(row[numberIndex]) || null,
...
function DataManagementWorkspace({ appliedData, draft, setDraft, canManage, actor, persist, db, showToast, draftKey }) {
  ...
  const removeMainSource = () => {
    if (!draft?.sourceName) return;
    if (!window.confirm("현재 임시 작업본에서 2학년부 기본 파일 자료를 삭제할까요? 소감문 파일은 유지됩니다.")) return;
    setDraft({
      ...draft,
      contacts: [],
      sourceName: "",
    ...
    showToast("임시 작업본에서 2학년부 기본 파일 자료를 삭제했습니다. 학교 반영본은 다시 반영하기 전까지 유지됩니다.", "success");
  };

  return <div className="gdt-data-workspace">
    <div className="gdt-data-grid">
      <article className="gdt-data-card applied"><header><span className="gdt-data-icon"><Database size={18}/></span><div><span className="gdt-data-eyebrow">학교 공용 자료</span><b>학교 반영본</b><span>담임과 2학년부 교사가 실제로 조회합니다.</span></div><em className="gdt-data-state">{appliedData ? "반영 완료" : "미반영"}</em></header><dl><div><dt>비상연락망</dt><dd>{appliedSummary.contacts}<small>명</small></dd></div><div><dt>생기부 활동</dt><dd>{appliedSummary.activities}<small>건</small></dd></div><div><dt>소감문</dt><dd>{appliedSummary.reflections}<small>건</small></dd></div></dl><footer><b>{appliedData?.sourceName || "반영된 기본 파일 없음"}</b><span>{appliedData?.appliedAt ? new Date(appliedData.appliedAt).toLocaleString("ko-KR") : "아직 학교 자료에 반영되지 않았습니다."}</span></footer></article>
      <article className="gdt-data-card draft"><header><span className="gdt-data-icon"><FileSpreadsheet size={18}/></span><div><span className="gdt-data-eyebrow">현재 브라우저 작업</span><b>현재 임시 작업본</b><span>검토·수정한 뒤 학교 자료에 반영합니다.</span></div><em className="gdt-data-state">편집 중</em></header><dl><div><dt>비상연락망</dt><dd>{draftSummary.contacts}<small>명</small></dd></div><div><dt>생기부 활동</dt><dd>{draftSummary.activities}<small>건</small></dd></div><div><dt>소감문</dt><dd>{draftSummary.reflections}<small>건</small></dd></div></dl><footer><b>{draft?.sourceName || "불러온 기본 파일 없음"}</b><span>학교 반영 전 임시 작업 상태입니다.</span></footer></article>
    </div>
    {!canManage ? <div className="gdt-info"><AlertTriangle size={15}/><span>자료 등록·교체는 2학년부장, 부서 계정 또는 관리자만 할 수 있습니다.</span></div> : <>
      ...
      <div className="gdt-source-list">
        ...
        {draft?.sourceName && <article className="main-source"><FileSpreadsheet size={15}/><span><b>{draft.sourceName}</b><small>2학년부 기본 업무 파일 · 연락망·활동·배정 자료</small></span><button type="button" onClick={removeMainSource}><Trash2 size={14}/>삭제</button></article>}
...
function ContactWorkspace({ data, accessRole, homeroomClass, showToast, onUpdateData, actor }) {
  ...
  const saveEdit = async () => {
    ...
    const nextContact = normalizeContactItem({
      ...
      classNumber: sidPlacement.classNumber || Number(editRow.classNumber),
      number: sidPlacement.number || Number(editRow.number),

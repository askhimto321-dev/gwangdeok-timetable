...
export default function MinimumCatalogAdmin({gdb,persistGrades,showToast}) {
 ...
 return <section className="kd-minimum-admin">
  <header><div><h3>수능최저 자료 연결</h3><p>학년도·캠퍼스·전형·모집단위 연결과 판정식을 분리해 관리합니다. 통사·통과는 2028 자료에서 사·과로 자동 보정합니다.</p></div><span>{gdb.minimumCatalog?.updatedAt?`최근 반영 ${new Date(gdb.minimumCatalog.updatedAt).toLocaleDateString('ko-KR')}`:'Patch92 기본 자료 적용 중'}</span></header>

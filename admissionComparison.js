...
export function resolveMinimumLink({target, data = {}, student, identity, evaluateMinimum, ambiguousType = false}) {
  // The cutoff is a 2026 result; the minimum shown to applicants is the
  // current 2028 rule (or the 2027 reference). They have separate source years.
  const catalog = resolveCatalogMinimum({target,student,identity});
  if (catalog && catalog.evaluation.status!=='unlinked') return catalog;
  // Legacy NAVI rows are a reference only when the current catalog has no
  // applicable rule. Preserve their source year in the evaluation.
